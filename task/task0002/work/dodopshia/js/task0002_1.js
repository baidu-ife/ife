...
function ipt1Fn(){
    ...
    if(data.indexOf(";")>-1){
        middata=data.split(";");
        console.log(middata);
        middata=uniqArray(middata);
        for(i=0;i<middata.length;i++){
            if(re.test(middata[i])){
            }else{
                datas.push(middata[i]);
            }
        }
    }else{
        datas.push(data);
    }
    $('#interest').innerHTML=datas;
...
function ta1Fn(){
    ...
    var re2=/^\s+?$/
    if(re.test(data)){
        middata=data.split(re);
        console.log(middata);
        middata=uniqArray(middata);
        for(i=0;i<middata.length;i++){
            if(re2.test(middata[i])){
            }else{
                datas.push(middata[i]);
            }
        }
...
function ta2Fn(){
    ...
    if(datas.length>0&&datas.length<=10){
        for(i=0;i<datas.length;i++){
            ...
            $('#interest3').appendChild(box);
            $('#interest3').appendChild(label);
        }
    }else{
