...
$.on('#ipt',"click",append);
    ...
    //$.on('#ipt','blur',lose);
    //这里有一个小bug，就是点击其他地方的时候，
    //下面那些suggestion应该消失,但是不知道应该怎么做，直接如下代码会有其他问题
...
function keyfn(event){
    ...
    if(event.keyCode==38){
        for(k=0;k<size;k++){
            $('#data').rows[k].bgColor="white";
        }
    if(rowNo==0){
        rowNo++;
    }
    $('#data').rows[--rowNo%size].bgColor="gray";
    $('#ipt').value=$('#data').rows[rowNo%size].innerText;
