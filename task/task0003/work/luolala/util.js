...
// your implement
   ...
   if (!element) return;
    ...
    if (elementClassName=='')
    {
     ...
     else if (elementClassName == newClassName || elementClassName.match(new RegExp("(^|\\s)" + newClassName + "(\\s|$)")))
        return;
...
// your implement
   ...
   $.on(element,eventName,function(event){
       ...
       {
           listener.call(target,e);//原来直接写listener会产生错误，在main.js的getClickStatus中console.log(this)
       }                         //显示为window。
   });
