...
function each(arr, fn) {// 实现一个遍历数组的方法，针对数组中每一个元素执行fn函数，并将数组索引和元素作为参赛传递
    for(var x=0; x<arr.length;x++) fn(arr[x], x);
}
...
function $(selector,father) {// 实现一个简单的Query,只返回一个元素
    ...
    if(selector.match(/^.[A-Za-z0-9_\-]+\b/)) return $(selector.replace(selector.match(/^.[A-Za-z0-9_\-]+\b\s*/),""),father.getElementsByClassName(selector.match(/^.([A-Za-z0-9_\-]+)\b/)[1]));
...
function setCookie(cookieName,cookieValue,expiredays)// 设置cookie
{
    ...
    var t = (expiredays==null) ? "" : ";expires="+exdate.toGMTString();
	document.cookie=cookieName+ "=" +escape(cookieValue)+t;
}
