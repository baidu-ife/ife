(function() {
	...
	Task.prototype.taskDone = function() {
		...
		todoDone.onclick = function() {
			...
			if (choosen) {
		 		...
		 		var f = g.att(choosen, "f"), t = g.att(choosen, "t"), todo = g.att(choosen, "todo"), result = g.timeChange(g.att(choosen, "time"));
		 		for (var i = 0; i < g.storage.todo[f][t].length; i ++) {
		 			if (g.storage.todo[f][t][i][0] == (todo + result[0] + result[1] + result[2])) {
		 				g.storage.todo[f][t][i][2] = 1;
		 				break;
		 			}
		 		}
		 		if (t != "folder") {
		 			for (var i = 0; i < g.storage.todo[f]["folder"].length; i ++) {
		 				if (g.storage.todo[f]["folder"][i][0] == (todo + result[0] + result[1] + result[2])) {
		 					g.storage.todo[f]["folder"][i][2] = 1;
		 				}
		 			}
		 		}
			}
	...
	Task.prototype.initTodo = function(f, t) {
		...
		for (var i = 0; i < time.length; i ++) {
			temp.push([time[i], todo[i], done[i]]);
		}
		...
		for (var i = 0; i < time.length; i ++) {
			...
			if (typeof done[i] != "string") {
				done[i] = done[i].toString();
			}
			var temp = todo[i].split(" "), tempDone = done[i].split(" ");
			for (var j = 0; j < temp.length; j ++) {
				if (tempDone[j] == 1) {
					str.push('<li class="todotaskLi done" f="' + f + '"todo="' + temp[j] + '"time="' + timeTemp[i]);
				}
				else {
	...
	Edit.prototype.save = function(type) {
		...
		if (type == "edit") {
			...
			if (!todochoosen) {
				alert("no choosen todo!");
				return false;
			}
			var	timeBefore = g.att(todochoosen, "time"), 
				t = g.att(todochoosen, 't'),
				f = g.att(todochoosen, 'f');
				titleBefore = g.att(todochoosen, "todo");
			result = g.timeChange(timeBefore);
			...
			if (t) {
				g.storage.todo[f][t].push([todo, content, '0']);
			}
			g.storage.todo[f]["folder"].push([todo, content, '0']);
		}
		task.initTodo(f, t);
		document.getElementsByClassName("edit")[0].innerHTML = this.normal;
		task.editTask();
